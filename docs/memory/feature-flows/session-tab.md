...
Voice mic, file upload, and SSE dynamic status labels are **deferred**. Each requires a backend extension (file payload on the turn endpoint, async_mode + SSE on the same) and `SessionPanel.vue` simply omits those affordances. Slash-command playbook autocomplete works automatically because `SessionPanel` reuses `ChatInput` with the agent name + status.
