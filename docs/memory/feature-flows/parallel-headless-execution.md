...
| Date | Changes |
|------|---------|
| 2026-04-10 | **Startup watchdog (#285)**: Added `_spawn_startup_watchdog()` helper in `claude_code.py` that kills the `claude` subprocess if it produces no stdout/stderr within `STARTUP_TIMEOUT_SECONDS` (default 60s, configurable via `CLAUDE_STARTUP_TIMEOUT` env var). Wired into both `execute_headless_task` and `execute_claude_code`. Converts hour-long zombie executions (caused by Claude Code hanging on expired OAuth tokens) into a 60s fast-fail with an actionable 503 error. Backend `task_execution_service.py` classifies the resulting 503 as `TaskExecutionErrorCode.AUTH` so downstream handlers can react. Slot release path is unchanged — existing `finally:` block in `execute_task` releases the slot on the new fail-fast path. |
